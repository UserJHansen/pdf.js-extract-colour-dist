...
describe("annotation", function () {
  ...
  describe("FreeTextAnnotation", () => {
    ...
    it("should render an added FreeText annotation for printing", async function () {
      ...
      const operatorList = await freetextAnnotation.getOperatorList(partialEvaluator, task, _util.RenderingIntentFlag.PRINT, false, null);
      expect(operatorList.fnArray.length).toEqual(16);
      expect(operatorList.fnArray).toEqual([_util.OPS.beginAnnotation, _util.OPS.save, _util.OPS.constructPath, _util.OPS.clip, _util.OPS.endPath, _util.OPS.beginText, _util.OPS.setTextMatrix, _util.OPS.setCharSpacing, _util.OPS.setFillRGBColor, _util.OPS.dependency, _util.OPS.setFont, _util.OPS.moveText, _util.OPS.showText, _util.OPS.endText, _util.OPS.restore, _util.OPS.endAnnotation]);
    });
  });
  describe("InkAnnotation", function () {
    ...
    it("should render an added Ink annotation for printing", async function () {
      ...
      const operatorList = await inkAnnotation.getOperatorList(partialEvaluator, task, _util.RenderingIntentFlag.PRINT, false, null);
      expect(operatorList.argsArray.length).toEqual(8);
      expect(operatorList.fnArray).toEqual([_util.OPS.beginAnnotation, _util.OPS.setLineWidth, _util.OPS.setLineCap, _util.OPS.setLineJoin, _util.OPS.setStrokeRGBColor, _util.OPS.constructPath, _util.OPS.stroke, _util.OPS.endAnnotation]);
      ...
      expect(operatorList.argsArray[4]).toEqual(new Uint8ClampedArray([0, 255, 0]));
      expect(operatorList.argsArray[5][0]).toEqual([_util.OPS.moveTo, _util.OPS.curveTo]);
